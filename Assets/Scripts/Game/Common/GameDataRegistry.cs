...
namespace Rootborn.Game.Common
{
    ...
    public sealed class GameDataRegistry : ScriptableObject
    {
        ...
        [SerializeField] private QuestChainDefinition[] _questChains = Array.Empty<QuestChainDefinition>();
        ...
        [SerializeField] private NpcRoleDefinition[] _npcRoles = Array.Empty<NpcRoleDefinition>();
        [SerializeField] private NpcScheduleDefinition[] _npcSchedules = Array.Empty<NpcScheduleDefinition>();
        ...
        [SerializeField] private CareerCandidateDefinition[] _careerCandidates = Array.Empty<CareerCandidateDefinition>();
        [SerializeField] private CareerHintDefinition[] _careerHints = Array.Empty<CareerHintDefinition>();
        [SerializeField] private CareerCandidateRouteDefinition[] _careerCandidateRoutes = Array.Empty<CareerCandidateRouteDefinition>();
        [SerializeField] private CareerInterestDefinition[] _careerInterests = Array.Empty<CareerInterestDefinition>();
        ...
        [SerializeField] private LocationCategoryDefinition[] _locationCategories = Array.Empty<LocationCategoryDefinition>();
        [SerializeField] private LocationDefinition[] _locations = Array.Empty<LocationDefinition>();
        [SerializeField] private LocationIdentityDefinition[] _locationIdentities = Array.Empty<LocationIdentityDefinition>();
        [SerializeField] private LocationActivityDefinition[] _locationActivities = Array.Empty<LocationActivityDefinition>();
        [SerializeField] private ExplorationInteractionDefinition[] _explorationInteractions = Array.Empty<ExplorationInteractionDefinition>();
        [SerializeField] private ExplorationChoiceDefinition[] _explorationChoices = Array.Empty<ExplorationChoiceDefinition>();
        [SerializeField] private ExplorationConditionBase[] _explorationConditions = Array.Empty<ExplorationConditionBase>();
        [SerializeField] private ExplorationOutcomeBase[] _explorationOutcomes = Array.Empty<ExplorationOutcomeBase>();
        [SerializeField] private ExplorationRiskPolicyDefinition[] _explorationRiskPolicies = Array.Empty<ExplorationRiskPolicyDefinition>();
        [SerializeField] private LocationStateDefinition[] _locationStates = Array.Empty<LocationStateDefinition>();
        [SerializeField] private LocationStateConflictPolicyDefinition[] _locationStateConflictPolicies = Array.Empty<LocationStateConflictPolicyDefinition>();
        [SerializeField] private DailyEventKindDefinition[] _dailyEventKinds = Array.Empty<DailyEventKindDefinition>();
        [SerializeField] private DailyEventDefinition[] _dailyEvents = Array.Empty<DailyEventDefinition>();
        [SerializeField] private CampaignDefinition[] _campaigns = Array.Empty<CampaignDefinition>();
        [SerializeField] private CampaignDayDefinition[] _campaignDays = Array.Empty<CampaignDayDefinition>();
        [SerializeField] private CampaignRouteDefinition[] _campaignRoutes = Array.Empty<CampaignRouteDefinition>();
        [SerializeField] private CampaignObjectiveBase[] _campaignObjectives = Array.Empty<CampaignObjectiveBase>();
        [SerializeField] private CampaignRewardBase[] _campaignRewards = Array.Empty<CampaignRewardBase>();
        ...
        [SerializeField] private FurnitureDefinition[] _furnitureDefinitions = Array.Empty<FurnitureDefinition>();
        ...
        [SerializeField] private WorldStateScopeDefinition[] _worldStateScopes = Array.Empty<WorldStateScopeDefinition>();
        [SerializeField] private WorldStateFlagDefinition[] _worldStateFlags = Array.Empty<WorldStateFlagDefinition>();
        [SerializeField] private WorldStateConditionBase[] _worldStateConditions = Array.Empty<WorldStateConditionBase>();
        [SerializeField] private WorldStateEffectBase[] _worldStateEffects = Array.Empty<WorldStateEffectBase>();
        [SerializeField] private WorldStateUsageDefinition[] _worldStateUsages = Array.Empty<WorldStateUsageDefinition>();
        [SerializeField] private WorldStateUsageConditionBase[] _worldStateUsageConditions = Array.Empty<WorldStateUsageConditionBase>();
        [SerializeField] private WorldStateUsageOutcomeBase[] _worldStateUsageOutcomes = Array.Empty<WorldStateUsageOutcomeBase>();
        [SerializeField] private WorldStateUsageRepeatPolicyDefinition[] _worldStateUsageRepeatPolicies = Array.Empty<WorldStateUsageRepeatPolicyDefinition>();
        [SerializeField] private DiscoveryClueDefinition[] _discoveryClues = Array.Empty<DiscoveryClueDefinition>();
        [SerializeField] private DiscoveryClueSourceDefinition[] _discoveryClueSources = Array.Empty<DiscoveryClueSourceDefinition>();
        [SerializeField] private DiscoveryClueConditionBase[] _discoveryClueConditions = Array.Empty<DiscoveryClueConditionBase>();
        [SerializeField] private DiscoveryClueCompletionBase[] _discoveryClueCompletions = Array.Empty<DiscoveryClueCompletionBase>();
        [SerializeField] private DiscoveryClueOutcomeBase[] _discoveryClueOutcomes = Array.Empty<DiscoveryClueOutcomeBase>();
        [SerializeField] private ClueInterpretationDefinition[] _clueInterpretations = Array.Empty<ClueInterpretationDefinition>();
        [SerializeField] private ClueInterpretationSourceDefinition[] _clueInterpretationSources = Array.Empty<ClueInterpretationSourceDefinition>();
        [SerializeField] private ClueInterpretationConditionBase[] _clueInterpretationConditions = Array.Empty<ClueInterpretationConditionBase>();
        [SerializeField] private ClueInterpretationOutcomeBase[] _clueInterpretationOutcomes = Array.Empty<ClueInterpretationOutcomeBase>();
        [SerializeField] private ClueInterpretationPolicyDefinition[] _clueInterpretationPolicies = Array.Empty<ClueInterpretationPolicyDefinition>();
        ...
        public QuestChainDefinition[] QuestChains => _questChains;
        ...
        public NpcRoleDefinition[] NpcRoles => _npcRoles;
        public NpcScheduleDefinition[] NpcSchedules => _npcSchedules;
        ...
        public CareerCandidateDefinition[] CareerCandidates => _careerCandidates;
        public CareerHintDefinition[] CareerHints => _careerHints;
        public CareerCandidateRouteDefinition[] CareerCandidateRoutes => _careerCandidateRoutes;
        public CareerInterestDefinition[] CareerInterests => _careerInterests;
        ...
        public TimeSlotDefinition[] TimeSlots => _timeSlots;
        public LocationCategoryDefinition[] LocationCategories => _locationCategories;
        public LocationDefinition[] Locations => _locations;
        public LocationIdentityDefinition[] LocationIdentities => _locationIdentities;
        public LocationActivityDefinition[] LocationActivities => _locationActivities;
        public ExplorationInteractionDefinition[] ExplorationInteractions => _explorationInteractions;
        public ExplorationChoiceDefinition[] ExplorationChoices => _explorationChoices;
        public ExplorationConditionBase[] ExplorationConditions => _explorationConditions;
        public ExplorationOutcomeBase[] ExplorationOutcomes => _explorationOutcomes;
        public ExplorationRiskPolicyDefinition[] ExplorationRiskPolicies => _explorationRiskPolicies;
        public LocationStateDefinition[] LocationStates => _locationStates;
        public LocationStateConflictPolicyDefinition[] LocationStateConflictPolicies => _locationStateConflictPolicies;
        public DailyEventKindDefinition[] DailyEventKinds => _dailyEventKinds;
        public DailyEventDefinition[] DailyEvents => _dailyEvents;
        public CampaignDefinition[] Campaigns => _campaigns;
        public CampaignDayDefinition[] CampaignDays => _campaignDays;
        public CampaignRouteDefinition[] CampaignRoutes => _campaignRoutes;
        public CampaignObjectiveBase[] CampaignObjectives => _campaignObjectives;
        public CampaignRewardBase[] CampaignRewards => _campaignRewards;
        public DiscoveryDefinition[] Discoveries => _discoveries;
        public PlaceableTileDefinition[] PlaceableTiles => _placeableTiles;
        public FurnitureDefinition[] FurnitureDefinitions => _furnitureDefinitions;
        ...
        public WorldStateScopeDefinition[] WorldStateScopes => _worldStateScopes;
        public WorldStateFlagDefinition[] WorldStateFlags => _worldStateFlags;
        public WorldStateConditionBase[] WorldStateConditions => _worldStateConditions;
        public WorldStateEffectBase[] WorldStateEffects => _worldStateEffects;
        public WorldStateUsageDefinition[] WorldStateUsages => _worldStateUsages;
        public WorldStateUsageConditionBase[] WorldStateUsageConditions => _worldStateUsageConditions;
        public WorldStateUsageOutcomeBase[] WorldStateUsageOutcomes => _worldStateUsageOutcomes;
        public WorldStateUsageRepeatPolicyDefinition[] WorldStateUsageRepeatPolicies => _worldStateUsageRepeatPolicies;
        public DiscoveryClueDefinition[] DiscoveryClues => _discoveryClues;
        public DiscoveryClueSourceDefinition[] DiscoveryClueSources => _discoveryClueSources;
        public DiscoveryClueConditionBase[] DiscoveryClueConditions => _discoveryClueConditions;
        public DiscoveryClueCompletionBase[] DiscoveryClueCompletions => _discoveryClueCompletions;
        public DiscoveryClueOutcomeBase[] DiscoveryClueOutcomes => _discoveryClueOutcomes;
        public ClueInterpretationDefinition[] ClueInterpretations => _clueInterpretations;
        public ClueInterpretationSourceDefinition[] ClueInterpretationSources => _clueInterpretationSources;
        public ClueInterpretationConditionBase[] ClueInterpretationConditions => _clueInterpretationConditions;
        public ClueInterpretationOutcomeBase[] ClueInterpretationOutcomes => _clueInterpretationOutcomes;
        public ClueInterpretationPolicyDefinition[] ClueInterpretationPolicies => _clueInterpretationPolicies;
