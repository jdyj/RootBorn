...
{
    public sealed class DiscoveryClueLogPanel : MonoBehaviour
    {
        ...
        public string TextForTests
        {
            get
            {
                ...
                {
                    var card = _cardsRoot.GetChild(i).GetComponent<DiscoveryClueCard>();
        ...
        public static DiscoveryClueLogPanel EnsureInScene(Canvas canvas)
        {
            var root = canvas != null ? canvas.transform.Find("DiscoveryClueLogPanel") : null;
            if (root != null)
            {
                var panel = root.GetComponent<DiscoveryClueLogPanel>();
                if (panel != null) return panel;
            }

            var go = new GameObject("DiscoveryClueLogPanel");
            if (canvas != null) go.transform.SetParent(canvas.transform, false);
            return go.AddComponent<DiscoveryClueLogPanel>();
        }
        ...
        public void Show(DiscoveryClueSummaryModel[] summaries)
        {
            BuildIfNeeded();
            ClearCards();
            var values = summaries ?? System.Array.Empty<DiscoveryClueSummaryModel>();
            for (int i = 0; i < values.Length; i++)
            {
                var card = DiscoveryClueCard.Create(_cardsRoot, "DiscoveryClueLogCard_" + i);
                var rt = (RectTransform)card.transform;
                rt.anchoredPosition = new Vector2(0f, -64f - i * 134f);
                card.Bind(values[i]);
            }
        ...
        private void BuildIfNeeded()
        {
            ...
            _root = new GameObject("DiscoveryClueLogRoot", typeof(RectTransform), typeof(ModernUiTileImage));
            ...
            rt.sizeDelta = new Vector2(450f, 560f);
            var tile = _root.GetComponent<ModernUiTileImage>();
            tile.SetRecipe(ModernUiRecipes.CommonPanel);
            tile.Rebuild();
            _title = MakeText(rt, "Title", "Clues", new Vector2(0f, 248f), new Vector2(360f, 32f), 22, TextAnchor.MiddleCenter);
            ...
            _cardsRoot.sizeDelta = new Vector2(370f, 480f);
        }
