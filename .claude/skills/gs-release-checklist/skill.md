...
description: "릴리스 체크리스트. 기존 unity-build-deploy 내부 호출. (Release checklist, wraps unity-build-deploy)"
---

<!--
한국어 요약: 릴리스 체크리스트. 기존 unity-build-deploy 내부 호출.
연계 에이전트: t2-release-manager, deploy-manager (기존)
연계 스킬: unity-build-deploy (internal), /gs-patch-notes
