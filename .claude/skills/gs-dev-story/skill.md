---
name: gs-dev-story
description: "스토리 구현. Phase 4 에서 dev-iteration-loop 호출하여 빌드·테스트·수정 반복. (Implement a story with programmer routing + dev-iteration-loop)"
---

<!--
한국어 요약: 스토리 구현. Phase 4 에서 dev-iteration-loop 호출하여 빌드·테스트·수정 반복.
연계 에이전트: t3-gameplay-programmer, t3-ui-programmer(P4b, fallback: t3-gameplay-programmer), t3-unity-specialist
연계 스킬: /gs-code-review, /gs-smoke-check, dev-iteration-loop (internal)
...
**Output:** Source code + test file in the project's `Assets/` and `Tests/` directories.
...
The agent should:
- Create or modify files in `Assets/` following the ADR guidelines
...
**Files changed**:
- `Assets/[path]` — created / modified ([brief description])
