---
name: gs-start
description: "경로 기반 파이프라인 라우팅. Assets=게임, server=speckit, 혼합=사용자 질의. (Entry point routing by paths)"
---

<!--
한국어 요약: 경로 기반 파이프라인 라우팅. Assets=게임, server=speckit, 혼합=사용자 질의.
연계 에이전트: 
연계 스킬: /gs-brainstorm, speckit.specify
...
- **Source code exists?** Glob for source files in `Assets/` (`*.gd`, `*.cs`, `*.cpp`, `*.h`, `*.rs`, `*.py`, `*.js`, `*.ts`).
...
- **User picks A but project has code**: Mention what you found — "I noticed there's already code in `Assets/`. Did you mean to pick D (existing work)?"
