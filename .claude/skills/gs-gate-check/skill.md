---
name: gs-gate-check
description: "Director 승인 게이트. /gs-design-system 후 필수. (Director gate)"
---

<!--
한국어 요약: Director 승인 게이트. /gs-design-system 후 필수.
연계 에이전트: t1-creative-director(P4b, fallback: t1-technical-director), t1-technical-director, t2-qa-lead
연계 스킬: /gs-create-epics
출처: Donchitos/Claude-Code-Game-Studios (MIT), 원문 영어, 경로 치환 적용
-->

> **Runtime directive**: If any linked agent above is marked `(P4b, fallback: X)`,
> and that agent file does not exist in `.claude/agents/`, delegate to `X` instead.
...
**Required Artifacts:**
- [ ] `Assets/` has active code organized into subsystems
- [ ] All core mechanics from GDD are implemented (cross-reference `design/gdd/` with `Assets/`)
...
- [ ] Localization strings are externalized (no hardcoded player-facing text in `Assets/`)
...
- For localization checks: `Grep` for hardcoded strings in `Assets/`

### Cross-Reference Checks
- Compare `design/gdd/` documents against `Assets/` implementations
