...
퀘스트, 미션, 스토리 진행, 지식 해금, 도구 해금처럼 인벤토리를 변경하는 보상 데이터는 지급 전 검증과 멱등성을 전제로 설계한다.

### 필수
- 보상 SO는 지급 대상 `ItemDefinition`, 수량, 조건, 완료 효과를 데이터로 표현한다.
- 런타임 지급 코드는 인벤토리 수용 가능 여부를 먼저 검사한 뒤, 모든 보상이 지급 가능할 때만 상태와 인벤토리를 함께 변경한다.
- 보상 상태는 `RewardClaimed` 같은 명시 상태로 저장되어야 하며, 저장/로드 후 같은 보상 지급 요청이 반복되어도 아이템이 다시 증가하지 않아야 한다.

### 금지
- 공간 확인 없이 `Inventory.Add` 를 먼저 호출한 뒤 실패 상태를 나중에 되돌리는 흐름.
- 여러 보상 중 일부만 지급된 상태에서 완료/수령 상태를 기록하는 부분 지급.
- 보상 수령 상태 없이 UI 버튼 비활성화만으로 이중 지급을 막는 방식.
