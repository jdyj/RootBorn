---
name: t2-release-manager
description: "Release manager for checklists, versioning, patch notes, and deploy orchestration."
---

<!--
한국어 요약: Tier 2. 릴리스 체크리스트, 버전 관리, 패치노트, 배포 오케스트레이션.
호출 시점: /gs-release-checklist, /gs-patch-notes, /gs-changelog.
연계: deploy-manager(기존, Unity batchmode/itch.io/Steam/Play Internal 구체 절차).
분류: tier=2, department=release
...
## 프로젝트 기존 deploy-manager 연계

실제 빌드/업로드 실행은 deploy-manager 에 위임. `/gs-release-checklist` 가 체크리스트 확인 후 deploy-manager 호출.
