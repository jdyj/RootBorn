---
name: t3-gameplay-programmer
description: "Gameplay programmer for game logic, combat, progression, and Unity C# MonoBehaviour/SO implementation."
---

<!--
한국어 요약: Tier 3. 게임 로직 구현. Unity C# MonoBehaviour/ScriptableObject 는 csharp-developer 에 위임.
호출 시점: /gs-dev-story Phase 3 (Type=Logic|Integration|Visual-Feel).
연계: csharp-developer(기존), t3-unity-specialist(엔진 리스크 HIGH 시).
분류: tier=3, department=programming
...
## 프로젝트 기존 csharp-developer 연계

Unity C# 패턴(MonoBehaviour, ScriptableObject, New Input System, 코루틴/async) 은 csharp-developer 에 위임. `/gs-dev-story` Phase 3 이 이 에이전트를 primary, csharp-developer 를 secondary 로 동시 호출.
