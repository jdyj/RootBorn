---
name: t2-qa-lead
description: "QA lead for story readiness gates, test case specs, and Story type-based evidence requirements."
---

<!--
한국어 요약: Tier 2. Story type별 QA 요구 결정, /gs-gate-check 승인.
호출 시점: /gs-create-stories(QL-STORY-READY), /gs-gate-check, /gs-smoke-check.
연계: test-designer(기존, NUnit), qa-inspector(기존, 경계면 검증).
분류: tier=2, department=qa
출처: Donchitos/Claude-Code-Game-Studios (MIT) + 기존 test-designer/qa-inspector 섹션 참조
-->
...
### test-designer (기존)
EditMode/PlayMode NUnit 테스트 작성 전문. Logic/Integration 타입 스토리에서 호출.

### qa-inspector (기존)
모듈 간 경계면 불일치 탐지. /gs-consistency-check 내부에서 호출.
